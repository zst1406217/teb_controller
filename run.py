...
if __name__ == "__main__":
    ...
    launch_file = join(base_path, '..', 'jackal_helper/launch/jackal_helper.launch')
    jackal_helper_process = subprocess.Popen([
        'roslaunch',
        launch_file,
    ])
    ...
    time.sleep(1)
    launch_file = join(base_path, '..', 'jackal_helper/launch/teb_rviz.launch')
    rviz_process = subprocess.Popen([
        'roslaunch',
        launch_file,
    ])
    horizon_process = subprocess.Popen([
        'python3',
        "horizon.py",
    ])
    time.sleep(3)
    os.system("rosparam set /move_base/TebLocalPlannerROS/publish_feedback true")
    nav_as.send_goal(mb_goal)
    teb_controller_process = subprocess.Popen([
        'python3',
        "teb_controller.py",
    ])
    # theta_transform_process = subprocess.Popen([
    #     'python2',
    #     "transform.py",
    # ])
    ...
    while compute_distance(init_coor, curr_coor) < 0.1:
       ...
       curr_coor = (pos.x, pos.y)
       time.sleep(0.01)
    #os.system("rosrun teb_controller teb_controller.py")
    ...
    if collided:
        status = "collided"
    elif curr_time - start_time >= 1000:
        status = "timeout"
    else:
        status = "succeeded"
        success = True
    ...
    with open(args.out, "a") as f:
        f.write("%d %d %d %d %.4f %.4f\n" %(args.world_idx, success, collided, (curr_time - start_time)>=1000, curr_time - start_time, nav_metric))
    
    # gazebo_process.terminate()
    # nav_stack_process.terminate()
    # rviz_process.terminate()
    # jackal_helper_process.terminate()
